...
export default function VerifyGhanaCardScreen() {
  ...
  const submit = async () => {
    if (!token) {
      return Alert.alert('Session Error', 'User session not initialized. Please sign in to complete verification.');
    }
    const card = number.replace(/\s/g, '').toUpperCase();
    if (!card || card.length < 10) return Alert.alert('Invalid card', 'Enter a valid Ghana Card number.');
    if (!asset) return Alert.alert('Image required', 'Choose a Ghana Card image before continuing.');
    setLoading(true);
    try {
      const verified = await verifyGhanaCard(card, asset);
      if (!verified) return Alert.alert('Not verified', 'We could not verify these details.');
  ...
  return (
    <View style={styles.container}>
      ...
      <Input placeholder="GHA-XXXXXXXXX-X" value={number} onChangeText={setNumber} autoCapitalize="characters" leftIcon="card-outline"/>
